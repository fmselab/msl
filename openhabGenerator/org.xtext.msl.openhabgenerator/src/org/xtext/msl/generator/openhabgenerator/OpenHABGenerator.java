...
public class OpenHABGenerator extends Generator {
					...
					for (ComponentInstance cI : cG.getComponents()) {
						for (AbstractComponent aC : aG.getComponents()) {
							if (cI.getType().equals(aC.getName()))
								// pw.println("Writing correspondence FROM " + cG.getName() + "." + cI.getName()
								// + " TO "+ aG.getName() + "." + aC.getName());
		...
		if (concrToAbsComponents != null) {
			...
			for (Interaction cI : configuration.getConcreteInteractions()) {
				for (AbstractInteraction aI : absInteractions) {
					...
					// pw.println(aI.getStart().getType().getName().equals(concrToAbsComponents.get(cI.getStart().getComponent()).getName())
					// &&
					// aI.getEnd().getType().getName().equals(concrToAbsComponents.get(cI.getEnd().getComponent()).getName()));
					if (aI.getStart().getType().getName()
						...
						// pw.println("Writing correspondence FROM " +
						// cI.getStart().getComponent().getName() + " -> " +
						// cI.getEnd().getComponent().getName() + " TO " +
						// aI.getStart().getType().getName() + " -> " +
						// aI.getEnd().getType().getName());
				...
				for (ComponentInstance cI : cG.getComponents()) {
					if (interactWithSameEnd.get(cI) != null) {
						...
						for (Interaction i : interactWithSameEnd.get(cI)) {
							...
							groupSwitchName = groupSwitchName
	...
	private void generateRules() {
		...
		if (writeLog && exec_number > 0) {
			...
			pw.println("when");
			pw.println(
					"\tItem start_" + concreteGroups.get(0).getComponents().get(0).getName() + " received command ON");
			pw.println("then");
			pw.println("\tif(exec_counter > 0) {");
			pw.println("\t\texec_counter = exec_counter-1");
			pw.println("\t\tlogInfo(\"" + configuration.getName()
					+ ".rules\", \"Execution counter reduced by one. exec_counter = \" + exec_counter)");
			pw.println("\t}");
			pw.println("\tif(exec_counter <= 0) {");
			pw.println("\t\tlogInfo(\"" + configuration.getName() + ".rules\", \"" + exec_number
					+ " executions have elapsed. Loops will end then stop.\")");
			pw.println("\t}");
			pw.println("end\n");
		}
		...
		if (writeLog) {
			if (exec_number > 0)
				pw.println("\tlogInfo(\"" + configuration.getName() + ".rules\", \"Loops will run for \" + "
						+ exec_number + " + \" iterations.\")");
			for (ConcreteGroup cG : concreteGroups) {
				pw.println("\tcounter_" + cG.getName() + " = 0");
				pw.println("\tlogInfo(\"" + configuration.getName() + ".rules\", \"Starting MAPE loop for group ("
						+ cG.getName() + "), counter_" + cG.getName() + " = \" + counter_" + cG.getName() + ")");
			}
			if (exec_number > 0)
				pw.println("\texec_counter = " + exec_number);
		}
		// initializes each MAPEloop. At runtime, a MAPEloop starts only if it has a
		// managed system to monitor, otherwise it has to wait until it is activated by
		// another loop
		for (ConcreteGroup cG : concreteGroups) {
			for (ComponentInstance cI : cG.getComponents()) {
		...
		for (ConcreteGroup cG : concreteGroups) {
			for (AbstractInteraction aI : absInteractions) {
				...
				if (writeLog) {
					...
					pw.println("\tlogInfo(\"" + configuration.getName() + ".rules\", \"" + ruleName + " for group ("
							+ cG.getName() + ") active, counter_" + cG.getName() + " = \" + counter_" + cG.getName()
							+ ")");
				}
				...
				if (aI.getStart().getType().getName().equals("A")) {
					...
					if (writeLog)
						pw.println("\t\tlogInfo(\"" + configuration.getName() + ".rules\", \""
								+ "Adaptation required for " + start + ", proceeding with loop.\")");
					pw.println("\t\tsendCommand(start_" + end + ", ON)");
					pw.println("\t}");
					pw.println("\telse{");
					if (writeLog) {
						pw.println("\t\tlogInfo(\"" + configuration.getName() + ".rules\", \""
								+ "Adaptation not required for " + start + ", resetting loop.\")");
						pw.println("\t\tcounter_" + cG.getName() + " = 0");
					}
						...
						for (ComponentInstance cI2 : cG.getComponents()) {
							if (interactWithSameEnd.get(cI2) != null) {
								...
								for (Interaction I : interactWithSameEnd.get(cI2)) {
									...
									pw.println(
											"\t\tsendCommand(start_" + I.getStart().getComponent().getName() + ", ON)");
								}
							}
						}
						if (endSimpleMAPE) {
...
//			if (cG.getManSys() != null) {
			/* if (cG.getManSys().size() != 0) {
				...
				pw.println("when");
				pw.println("\tItem start_" + "e_" + cG.getName() + " received command ON"); //TODO: anzicch� prendere il name di cG, cercare all'interno del cG il nome della componente M
				pw.println("then");
				if (writeLog) {
					pw.println("\tcounter_" + cG.getName() + " = counter_" + cG.getName() + " + 1");
	...
	// determines whether the abstract interaction aI runs within the concretegroup
	// cG
	...
	// prints all interactions where the group of the start component is different
	// that that of the end component
			...
			for (ComponentInstance cI : cG.getComponents()) {
				if (interactWithSameEnd.get(cI) != null) {
					...
					for (Interaction i : interactWithSameEnd.get(cI)) {
						...
						sendCommandSwitches.add(compToGroupConcrete.get(i.getStart().getComponent()).getName() + "_to_"
								+ compToGroupConcrete.get(i.getEnd().getComponent()).getName());
					}
					...
					for (Interaction i : interactWithSameEnd.get(cI)) {
						...
						pw.println("then");
						if (writeLog) {
							pw.println("\tcounter_" + compToGroupConcrete.get(i.getStart().getComponent()).getName()
									+ " = counter_" + compToGroupConcrete.get(i.getStart().getComponent()).getName()
									+ " + 1");
							pw.println("\tlogInfo(\"" + configuration.getName() + ".rules\", \"" + GetRuleName(i)
									+ " for group (" + compToGroupConcrete.get(i.getStart().getComponent()).getName()
									+ ") active, counter_"
									+ compToGroupConcrete.get(i.getStart().getComponent()).getName()
									+ " = \" + counter_"
									+ compToGroupConcrete.get(i.getStart().getComponent()).getName() + ")");
						}
						pw.println("\tsendCommand(start_" + i.getStart().getComponent().getName() + ", OFF)");
						pw.println("\tsendCommand(" + compToGroupConcrete.get(i.getStart().getComponent()).getName()
								+ "_to_" + compToGroupConcrete.get(i.getEnd().getComponent()).getName() + ", ON)");
						pw.println("end\n");
					}
					...
					if (writeLog) {
						...
						pw.print("\tlogInfo(\"" + configuration.getName() + ".rules\", \"Aggregating output from "
								+ logStarts + "to " + endName + "\"");
	...
	public static void main(String[] args) {
		...
		// Default: true.
		/*
		 * you'll find some specifications in examples/: SimpleMAPE.msl : A single
		 * MAPEloop without other dependencies SimpleMAPE_MissingComp.msl : A MAPEloop
		 * without Analyze and Plan SimpleMAPE_MultipleLoops.msl : Two independent
		 * MAPEloops running in parallel AggregateMAPE.msl : Two MAPEloops, one managing
		 * the system, depending on the other with AP functions
		 */
		// output files go in conf/items and conf/rules; naming conventions are the same
		// as those found in the OpenHAB folder
